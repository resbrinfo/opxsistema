...
// Processa o cadastro da Administradora
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salvar_admin'])) {
    try {
        $nome = filter_input(INPUT_POST, 'nome', FILTER_SANITIZE_SPECIAL_CHARS);

        if (!empty($nome)) {
            $stmt = $pdo->prepare("INSERT INTO administradoras (nome) VALUES (:nome)");
            $stmt->execute([':nome' => $nome]);
            $mensagem_swal = "Swal.fire('Sucesso', 'Administradora cadastrada com sucesso!', 'success');";
        }
...
// Busca Administradoras
$administradoras = [];
try {
    $administradoras = $pdo->query("SELECT * FROM administras ORDER BY nome ASC")->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {}
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <title>Cadastro de Administradoras - Premium Clean</title>
...
<body>
    ...
    <div class="main-content">
        <div class="card-premium">
            <h2 style="margin:0 0 1.5rem 0; font-size: 1.4rem;">Cadastro de Administradoras de Benefícios</h2>
            <form action="" method="POST">
                <div style="display: grid; grid-template-columns: 1fr auto; gap: 1rem; align-items: flex-end;">
                    <div class="form-group" style="margin:0;">
                        ...
                        <input type="text" name="nome" class="form-control" placeholder="Ex: Qualicorp, Benevix, Corpore" required>
                    </div>
                    <button type="submit" name="salvar_admin" class="btn-premium-orange"><i class="fa-solid fa-plus"></i> Cadastrar</button>
                </div>
            </form>
        </div>

        <div class="card-premium">
            <h3 style="margin:0 0 1.5rem 0; font-size: 1.1rem;">Administradoras Ativas</h3>
            <table class="table-premium">
                ...
                <tbody>
                    <?php if(count($administradoras) > 0): ?>
                        <?php foreach($administradoras as $ad): ?>
                            <tr>
                                <td>#<?= $ad['id'] ?></td>
                                <td><strong><?= htmlspecialchars($ad['nome']) ?></strong></td>
                                <td><span class="badge"><?= $ad['status'] ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="3" style="text-align: center; color: var(--texto-mutado); padding: 2rem;">Nenhuma administradora cadastrada.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
