...
// ==============================================================================
// 1. PROCESSAMENTO: CADASTRO DE SEGURO DE VIDA + DEPENDENTES DINÂMICOS (POST)
// ==============================================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salvar_vida'])) {
    ...
    // Arrays dos dependentes injetados dinamicamente
    ...
    // Limpa a moeda do titular
    $valor_titular = str_replace(['.', ','], ['', '.'], $valor_titular);

    if (!empty($cliente_id) && !empty($corretor_id) && !empty($grade_comissao_id) && !empty($num_apolice)) {
        try {
            $pdo->beginTransaction();

            // Inserção do Contrato Mãe (Titular) incluindo a grade relacional de comissão
            $stmtVida = $pdo->prepare("
                INSERT INTO vida (cliente_id, corretor_id, grade_comissao_id, num_apolice, plano, valor_titular, tem_dependentes) 
                VALUES (:cliente_id, :corretor_id, :grade_comissao_id, :num_apolice, :plano, :valor_titular, :tem_dep)
            ");
            $stmtVida->execute([
            ...
            // Captura o ID do contrato recém-inserido
            $vida_id = $pdo->lastInsertId();

            // Inserção dos Dependentes Vinculados (Se houver)
            if ($tem_dependentes === 'Sim' && !empty($dep_nomes)) {
                $stmtDep = $pdo->prepare("
                    ...
                    VALUES ('Vida', :contrato_id, :nome, :cpf, :nasc, :parentesco, :valor)
                ");
            ...
            $pdo->commit();
            $mensagem_swal = ['status' => 'success', 'title' => 'Sucesso!', 'text' => 'Apólice de Vida lançada com sucesso com seus dependentes.'];
        } catch (Exception $e) {
            if ($pdo->inTransaction()) { $pdo->rollBack(); }
            error_log("Erro ao cadastrar seguro de vida: " . $e->getMessage());
            $mensagem_swal = ['status' => 'error', 'title' => 'Erro', 'text' => 'Falha técnica ao salvar apólice: ' . $e->getMessage()];
        }
    }
}
...
// ==============================================================================
try {
    // Busca a carteira de Vida cruzando Clientes, Corretores e calculando o total de dependentes por apólice
    $queryVida = "
        SELECT 
            v.*, 
            cli.nome_completo as nome_cliente,
            corr.nome_completo as nome_corretor,
            (SELECT COUNT(*) FROM dependentes d WHERE d.contrato_id = v.id AND d.tipo_contrato = 'Vida') as qtd_dependentes
        FROM vida v
        INNER JOIN clientes cli ON v.cliente_id = cli.id
        INNER JOIN corretores corr ON v.corretor_id = corr.id
        ORDER BY v.id DESC
    ";
    $vidasListadas = $pdo->query($queryVida)->fetchAll(PDO::FETCH_ASSOC);
    $totalVidas = count($vidasListadas);
    ...
    // Nova consulta para carregar as grades de comissão ativas no sistema
    $gradesSelect     = $pdo->query("SELECT id, nome_grade FROM grades_comissoes ORDER BY nome_grade ASC")->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $vidasListadas = []; $clientesSelect = []; $corretoresSelect = []; $gradesSelect = []; $totalVidas = 0;
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Seguros de Vida | Egis Saúde</title>
...
<div class="main-content">
    <div class="d-flex justify-content-between align-items-center mb-5 flex-wrap gap-3">
        <div>
            <h2 class="fw-bold text-dark mb-1">Ramo Vida (Pessoas)</h2>
            <p class="text-muted mb-0">Controle capitais segurados, planos individuais, apólices mãe e grupos de dependentes.</p>
        </div>
        <button class="btn btn-dark rounded-3 px-4 py-2 fw-500" data-bs-toggle="modal" data-bs-target="#modalNovoVida" style="background:#ea580c; border:none; box-shadow: 0 4px 12px rgba(234, 88, 12, 0.2);">
            <i class="fa-solid fa-heart-pulse me-2"></i> Emitir Apólice Vida
        </button>
    </div>
    ...
    <div class="row g-4 mb-5">
        <div class="col-md-4">
            <div class="card-premium py-3">
                <span class="text-uppercase text-muted font-monospace small fw-bold">Vidas Seguradas</span>
                <h3 class="fw-bold text-dark mt-1 mb-0"><?= $totalVidas ?> Contratos</h3>
            </div>
        </div>
    </div>
    ...
    <div class="card-premium">
        <div class="mb-4">
            <input type="text" class="form-control search-box w-25" id="filtroVida" onkeyup="filtrarVida()" placeholder="Buscar por segurado ou plano...">
        </div>
        
        <div class="table-responsive">
            <table class="table table-borderless align-middle mb-0 table-premium" id="tabelaVida">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php if (empty($vidasListadas)): ?>
                        <tr>
                            <td colspan="6" class="text-center text-muted py-5">Nenhum seguro de vida emitido na base.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($vidasListadas as $vid): ?>
                            <tr>
                                <td data-label="Segurado Titular" class="fw-bold text-dark">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <?= htmlspecialchars($vid['nome_cliente']) ?>
                                    </div>
                                </td>
                                <td data-label="Nº Apólice" class="font-monospace text-secondary small fw-bold">
                                    <?= htmlspecialchars($vid['num_apolice']) ?>
                                </td>
                                <td data-label="Plano" class="text-dark fw-medium">
                                    <?= htmlspecialchars($vid['plano'] ?? 'Não informado') ?>
                                </td>
                                <td data-label="Custo Titular" class="fw-bold text-dark">
                                    R$ <?= number_format($vid['valor_titular'], 2, ',', '.') ?>
                                </td>
                                <td data-label="Dependentes">
                                    <?php if ($vid['tem_dependentes'] === 'Sim'): ?>
                                        <span class="badge bg-orange-premium rounded-pill px-2.5 py-1.5 fw-medium" style="background: rgba(234, 88, 12, 0.08); color:#ea580c; font-size:0.72rem;">
                                            <i class="fa-solid fa-child me-1"></i> Sim (<?= $vid['qtd_dependentes'] ?>)
                                        </span>
                                    <?php else: ?>
                                        <span class="badge bg-light text-muted rounded-pill px-2.5 py-1.5 fw-medium" style="font-size:0.72rem;">Não possui</span>
                                    <?php endif; ?>
                                </td>
                                <td data-label="Corretor" class="text-muted small fw-medium">
                                    <i class="fa-solid fa-user-tie me-1 opacity-50"></i> <?= htmlspecialchars($vid['nome_corretor']) ?>
                                e/td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
<div class="modal fade" id="modalNovoVida" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4" style="background: #ffffff;">
            <div class="modal-header border-bottom border-light p-4">
                <h5 class="modal-title fw-bold text-dark"><i class="fa-solid fa-heart-circle-check text-orange-premium me-2" style="color:#ea580c;"></i> Lançar Seguro de Vida</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="vida.php" method="POST">
                <div class="modal-body p-4" style="max-height: 70vh; overflow-y: auto;">
                    
                    <h6 class="text-uppercase tracking-wider text-muted font-monospace mb-3" style="font-size:0.75rem; font-weight:700;">1. Dados da Apólice Principal & Grade</h6>
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Corretor de Vendas</label>
                            <select name="corretor_id" class="form-select rounded-3" required>
                                <option value="" disabled selected>Vincular parceiro corporativo...</option>
                        ...
                        <div class="col-md-4">
                            <label class="form-label">Nº da Apólice</label>
                            <input type="text" name="num_apolice" class="form-control rounded-3" placeholder="Ex: AP-883291" required>
                        </div>
                        <div class="col-md-5">
                            <label class="form-label">Denominação do Plano</label>
                            <input type="text" name="plano" class="form-control rounded-3" placeholder="Ex: Vida Individual Premium, Global Coletivo" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Prêmio Titular (R$)</label>
                    ...
                    <h6 class="text-uppercase tracking-wider text-muted font-monospace mb-2" style="font-size:0.75rem; font-weight:700;">2. Estrutura de Dependentes</h6>
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            <label class="form-label">Possui Dependentes / Beneficiários?</label>
                            <select name="tem_dependentes" id="selectDep" class="form-select rounded-3" onchange="togglePainelDependentes()">
                                ...
                                <option value="Sim">Sim, incluir dependentes</option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="modal-footer border-top border-light p-4">
                    ...
                    <button type="submit" name="salvar_vida" class="btn btn-primary rounded-3 px-4 py-2" style="background:#ea580c; border:none;">Salvar Contrato</button>
                </div>
            </form>
        </div>
    </div>
...
// Controls visual display of the dependents panel
function togglePainelDependentes() {
    ...
    if (status === 'Sim') {
        ...
        if (document.getElementById('listaDependentes').children.length === 0) {
            adicionarLinhaDependente(); // Insere a primeira linha em branco automaticamente
        }
    } else {
...
// Injects an entire sub-form row for dependents via pure JS
function adicionarLinhaDependente() {
    ...
    div.innerHTML = `
        <div class="d-flex justify-content-between align-items-center mb-3">
            <span class="badge bg-dark text-white font-monospace">Dependente #${index + 1}</span>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="dep_parentesco[]" class="form-control form-control-sm rounded-2" placeholder="Ex: Cônjuge, Filho(a), Pai" required>
            </div>
            <div class="col-md-6">
                <label class="form-label small text-muted">Prêmio Individual do Dependente (R$)</label>
...
function filtrarVida() {
    const filter = document.getElementById("filtroVida").value.toUpperCase();
    const rows = document.getElementById("tabelaVida").getElementsByTagName("tr");
    for (let i = 1; i < rows.length; i++) {
        let tdCliente = rows[i].getElementsByTagName("td")[0];
        let tdPlano   = rows[i].getElementsByTagName("td")[2];
